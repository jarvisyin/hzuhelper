...
public class ManaTakeoutMenu extends ServletBase {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			ArrayList<TakeoutMenu> list = tmigbll.getList(restaurantId);
			TakeoutRestaurant1 model = trbll.getModel(restaurantId);
	...
	private void add(HttpServletRequest request, PrintWriter out) {
		...
		try {
			if (name == null) {
				...
				TakeoutMenu model = new TakeoutMenu();
